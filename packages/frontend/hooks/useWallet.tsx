...
const getBalances = (account) => {
  ...
  return erc20Tokens.reduce(
    (r, { name, address, useBalance }) => {
      const balance = useBalance(address, account)
      r[name] = balance ? parseFloat(utils.formatEther(balance).slice(0, 8)) : 0
      return r
    },
    {
      [CURRENCY_ETH]: ethBalance
        ? parseFloat(utils.formatEther(ethBalance).slice(0, 8))
        : 0,
    }
  )
}
